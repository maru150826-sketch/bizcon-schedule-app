...
- 確定した作業予定を画面上部に表示
- 確定日時ごとに共有メモを保存可能
- 確定日時で、空き時間未入力の人も「参加できる / 一部・要相談 / 参加できない」を追加回答可能
- 確定日時ごとの参加状況を、参加・一部・不参加・未回答で表示

Supabaseはv11以降のSQLを実行済みなら基本そのまま動きます。
responses と meeting_notes テーブルを使うため、未作成の場合は supabase-schema.sql を再実行してください。
